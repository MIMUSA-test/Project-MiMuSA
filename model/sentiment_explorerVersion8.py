...
def newtext_fullstop(text):
    ...
    text = text.replace('<br>', '')  # Remove breaks
    text = re.sub('".*?"', '', text)  # delete anything in quotation marks
    text = re.sub('http[s]?://\S+', '', text) # delete urls
    text = re.sub(r'\b(?:www\.)?[A-Za-z0-9_-]+\.(?:com|org|net|gov|edu|info|biz|co)\b', '', text) # delete websites such as JD.com
    text = re.sub(r'\b\d+\.\d+\b', '', text) # deletes decimal numbers
...
def breakParagraph(para):
    ...
    each sentence to be evaluated on polarity.
...
##------------------------------------------------Analyst Score Sentiment Interpretation ------------------------------------------------##

def analyst_sentiment(value):
...
def analyst_basic_sentiment(complex_sentiment):
    basic_sentiment = "Undetermined"
    if complex_sentiment=="Neutral":
        basic_sentiment = "Neutral"
    elif "Positive" in complex_sentiment:
        basic_sentiment = "Positive"
    elif "Negative" in complex_sentiment:
